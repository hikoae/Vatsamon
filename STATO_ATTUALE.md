...
- PWA: manifest + icone + offline (workbox, cache tile OSM). `base` = `/vazzamon/` (GitHub Pages). Workflow `.github/workflows/deploy.yml`.

## Struttura `app/src/` (3554 righe)
| File | righe | ruolo |
|---|---|---|
| `App.tsx` | **2584** | **monolite**: tutta la logica + tutte le 5 viste |
| `data/vazzadex.json` | 2098 | 73 bovine reali + 6 pascoli |
| `components/VazzamonAvatar.tsx` | 309 | avatar mucca SVG procedurale (per generati) |
...
| `data/mockVazzamon.ts` | 145 | dati v2: 2 starter, 3 boss demo, 5 hotspot |
| `data/realCows.ts` | 80 | **convertitore** DB reale → modello Vazzamon |
| `types.ts` | 91 | tipi (Vazzamon esteso con campi reali) |
...
Viste: MAP / AR SCAN / EGGS(Vitelli) / VAZZADEX / BATTLE(Gym).

## Schermate (5 tab)
`map` (Mappa) · `scanner` (AR Scan) · `eggs` (Vitelli) · `vazzadex` · `battle` (Gym).

## Stato salvato (localStorage) — 6 chiavi
`vazzamon_collection_go` (collezione) · `vazzamon_bag_go` (zaino) ·
`vazzamon_eggs_go` (uova) · `vazzamon_trainer_go` (allenatore) ·
`vazzamon_waypoint_idx` + `vazzamon_waypoint_progress` (posizione sul sentiero).
> Nessuno store/context: tutto è `useState` dentro `App.tsx` con `useEffect` di persistenza.

## Modello dati `Vazzamon` (types.ts)
...
3. **Vazzadex**: card "Reines reali: X/73" + nella scheda **4 statistiche reali** e provenienza (comune/allevatore/peso/riconoscimento) quando `isReal`.
4. **CowVisual**: foto reale nei marker, cattura, griglia, dettaglio, arena.
5. **Scanner**: `generateVazzamonClient` (statico, niente server/chiavi). Camera reale via getUserMedia funziona; l'analisi è simulata.

## Verificato (Playwright, `npm run verify`)
Header VAZZAMON GO · 5 tab navigabili · 73 marker reali sulla mappa · GPS si attiva ·
...
- ✅ **Avvio pulito**: Vazzadex vuota (0/73), allenatore a 0 catture/XP. Niente più "+2 bonus IA" fasullo.
- ✅ **Avversari Gym = vere Reines** (Epiche/Leggendarie reali, con foto nell'arena).
- ✅ Rimosso `src/data/mockVazzamon.ts` (interamente inutilizzato dopo i fix).
...
- Restano le **selvatiche IA** procedurali di v2 e lo scanner: i loro Vazzamon contano come "bonus IA" nel conteggio collezione (separati dalle 73 reali).

## Repo
- `app/` = build attiva. `materiali/` = sorgenti/riferimenti (incluso `vazzamon_v2` originale, `foto`, dataset, prompt, challenge).
- Backup su GitHub: `git@github.com:hikoae/vazzamon.git`, branch `main` (pushato).
