...
// Cultural lists of Valle d'Aosta Vatsamon parameters for fallback simulation
const AUTUMN_BREEDS = [
...
// Helper to generate a procedural randomized fallback Vatsamon in Italian
function simulateVatsamon(): any {
...
// API endpoint for generating Vatsamon from scan / upload
app.post("/api/generate-vatsamon", async (req, res) => {
  try {
    const { imageBase64, isDemo = false } = req.body;

    if (isDemo) {
      // Instantly generate a premium high-stat Vatsamon for demonstration
      console.log("Serving simulated Vatsamon for DEMO request");
      return res.json({
...
Il tuo compito è analizzare la foto o l'immagine fornita. Se l'immagine contiene un elemento naturale (mucca, prato, baita, cibo alpino, pietra alpina ecc.) usalo per coniare un Vatsamon personalizzato basato sul DNA del territorio alpino.
Se l'immagine non è pertinente, immagina un simpatico Vatsamon ispirato all'energia dell'immagine inserita.
...
Devi generare un file JSON che rispetta accuratamente il seguente schema stringente:
{
  ...
  "lore": "Descrizione narrativa in italiano, spiritosa ed epica sul legame di questa mucca con il territorio alpino"
    ...
    if (imageBase64) {
      ...
      contents.push({
        text: "Analizza questa impronta di DNA o foto ed estrai il leggendario Vatsamon racchiuso all'interno. Fornisci un output rigoroso in formato JSON."
      });
    } else {
      contents.push({
        text: "Genera un Vatsamon a sorpresa emerso dalle vette del Gran Paradiso o dal Monte Bianco. Fornisci un output rigoroso in formato JSON."
      });
    }
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Vatsamon Server fully operational at http://localhost:${PORT}`);
  });
}
