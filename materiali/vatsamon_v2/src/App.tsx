...
export default function App() {
  ...
  }, [activeTab, mapMode, playerLat, playerLng, wildCows, caseraCooldowns]);
                  ...
                  {vatsadex.length > 0 ? (
                    <div className="flex flex-col items-center space-y-4">
                      ...
                      <div className="bg-slate-950 rounded-2xl p-4 border border-slate-850 flex items-center gap-4 w-[80%]">
                        <VatsamonAvatar breed={(vatsadex.find(c => c.id === activeCombatantId) || vatsadex[0]).breed} rarity={(vatsadex.find(c => c.id === activeCombatantId) || vatsadex[0]).rarity} className="w-16 h-16" />
                        <div className="text-left">
                          <h4 className="font-mono font-black text-slate-200">{(vatsadex.find(c => c.id === activeCombatantId) || vatsadex[0]).name}</h4>
                          <span className="text-[10px] bg-slate-900 border border-slate-800 text-yellow-400 py-0.5 px-2 rounded font-mono font-bold">
                            CP {(vatsadex.find(c => c.id === activeCombatantId) || vatsadex[0]).cp}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-5 bg-slate-950/80 border border-slate-850 rounded-3xl p-5 items-center gap-4 relative overflow-hidden shadow-2xl">
                    ...
                    <div className="md:col-span-2 bg-slate-900 rounded-2xl p-4 border border-slate-800 relative space-y-2 text-center overflow-hidden">
                      ...
                      <div className={`transition-all duration-150 flex justify-center ${gymState.playerAttackAnim ? 'translate-x-6 scale-110 rotate-3 z-10' : ''} ${activeDodgeEffect ? '-translate-y-4 filter blur-xs duration-75' : ''}`}>
                        <VatsamonAvatar breed={gymState.playerVatsamon!.breed} rarity={gymState.playerVatsamon!.rarity} className="w-20 h-20" />
                      </div>

                      <h4 className="font-mono font-black text-yellow-300 text-xs truncate uppercase leading-none mt-1">{gymState.playerVatsamon!.name}</h4>
                    ...
                    <div className="md:col-span-2 bg-slate-900 rounded-2xl p-4 border border-slate-800 relative space-y-2 text-center overflow-hidden">
                      ...
                      <div className={`transition-all duration-150 flex justify-center ${gymState.opponentAttackAnim ? '-translate-x-6 scale-110 -rotate-3 z-10' : ''}`}>
                        <VatsamonAvatar breed={gymState.opponentVatsamon!.breed} rarity={gymState.opponentVatsamon!.rarity} className="w-20 h-20" />
                      </div>

                      <h4 className="font-mono font-black text-red-400 text-xs truncate uppercase leading-none mt-1">{gymState.opponentVatsamon!.name}</h4>
