...
import { INITIAL_VATSADEX, DEMO_PREMIUM_COWS, HP_LOCATIONS } from './data/mockVatsamon';
...
export default function App() {
  ...
  const [backpack, setBackpack] = useState<BackpackItem[]>(() => {
    ...
    return [
      { id: 'item-bell-std', name: 'Vatsa-ball Ottone', description: 'Campanaccio base risonante. Ideale per catture ordinarie.', quantity: 20, type: 'ball' },
      ...
      { id: 'item-apple', name: 'Mela Alpina d\'Oro', description: 'Frutto profumatissimo. Addolcisce i Vatsamon selvatici del 40%.', quantity: 6, type: 'food' },
  ...
  const [activeRouteId, setActiveRouteId] = useState<string>(() => {
    return localStorage.getItem('vatsamon_active_route_id') || 'alta_via_1';
  });

  // Trainer gym badges system
  const [trainerBadges, setTrainerBadges] = useState<string[]>(() => {
    const cached = localStorage.getItem('vatsamon_badges');
    if (cached) {
      try { return JSON.parse(cached); } catch (e) { console.error(e); }
    }
    return [];
  ...
  useEffect(() => {
    localStorage.setItem('vatsamon_active_route_id', activeRouteId);
  }, [activeRouteId]);
  ...
  const [currentWaypointIndex, setCurrentWaypointIndex] = useState<number>(() => {
    const cached = localStorage.getItem(`vatsamon_waypoint_idx_${activeRouteId}`);
    return cached ? Number(cached) : 0;
  });
  const [waypointProgress, setWaypointProgress] = useState<number>(() => {
    const cached = localStorage.getItem(`vatsamon_waypoint_progress_${activeRouteId}`);
    return cached ? Number(cached) : 0;
  });

  // Synchronize dynamic indices when route switches
  useEffect(() => {
    const cachedIdx = localStorage.getItem(`vatsamon_waypoint_idx_${activeRouteId}`);
    const cachedProg = localStorage.getItem(`vatsamon_waypoint_progress_${activeRouteId}`);
    setCurrentWaypointIndex(cachedIdx ? Number(cachedIdx) : 0);
    setWaypointProgress(cachedProg ? Number(cachedProg) : 0);
  }, [activeRouteId]);

  useEffect(() => {
    localStorage.setItem(`vatsamon_waypoint_idx_${activeRouteId}`, String(currentWaypointIndex));
  }, [currentWaypointIndex, activeRouteId]);

  useEffect(() => {
    localStorage.setItem(`vatsamon_waypoint_progress_${activeRouteId}`, String(waypointProgress));
  ...
  // ---- 2. VIEW NAVIGATION ----
  const [activeTab, setActiveTab] = useState<'map' | 'scanner' | 'eggs' | 'vatsadex' | 'battle'>('map');
  ...
  const executeOpponentTurn = (currentState: BattleState) => {
    ...
    setTimeout(() => {
      setGymState(prev => {
        if (prev.status !== 'active' || !prev.opponentVatsamon) return prev;

        const isSuperReady = prev.opponentEnergy >= 100;
        let baseDamage = 13 + (prev.opponentVatsamon.stats.strength * 0.15) + (Math.random() * 8);
        ...
        if (isHeal) {
          const healAmount = Math.floor(45 + prev.opponentVatsamon.stats.agility * 0.35);
          const nextOppHp = Math.min(prev.opponentMaxHp, prev.opponentHp + healAmount);
          logs.push(`${moveEmoji} Il Boss ${prev.opponentVatsamon.name} si ritira in difesa e usa ${moveName}! Recupera +${healAmount} HP.`);
          ...
          if (isPlayerEvaded) {
            logs.push(`💨 Fantastico! Il tuo ${prev.playerVatsamon?.name} elude e schiva con stile l'attacco ${moveName}! Nessun danno subìto.${trainerBadges.includes('morgex') ? " (Schivata Vigneto)" : ""}`);
            playClickSfx();
            ...
            logs.push(`${moveEmoji} Boss ${prev.opponentVatsamon.name} scaglia ${moveName}! Infligge ${finalDamage} danni.`);
            playHitSfx();
            ...
            if (nextPlayerHp <= 0) {
              status = 'ended';
              winner = 'opponent';
              logs.push(`💀 Il tuo leale compagno ${prev.playerVatsamon?.name} è esausto! Ha dato tutto nell'arena d'alta quota.`);
            }
  ...
  const handleExecutePlayerMove = (moveId: 'headbutt' | 'shield' | 'healing' | 'special') => {
    ...
    setGymState(prev => {
      if (!prev.playerVatsamon || !prev.opponentVatsamon) return prev;
      ...
      if (moveId === 'headbutt') {
        damage = Math.round(16 + (prev.playerVatsamon.stats.strength * 0.18) + Math.random() * 6);
        energyGain = 18;
        description = `sferra TESTATA D'ALTA QUOTA 💥`;
        playHitSfx();
      } else if (moveId === 'shield') {
        damage = Math.round(7 + (prev.playerVatsamon.stats.strength * 0.08) + Math.random() * 4);
        energyGain = 12;
        ...
        let baseHeal = Math.round(55 + (prev.playerVatsamon.stats.agility * 0.4) + arenaCologneHealBonus);
        ...
        damage = Math.round((26 + prev.playerVatsamon.stats.strength * 0.25) * 2.3);
        energyGain = -100;
      ...
      if (isHeal) {
        ...
        logs.push(`🍀 ${prev.playerVatsamon.name} ${description}.`);
      } else if (isOppEvaded) {
        logs.push(`💨 Boss ${prev.opponentVatsamon.name} scarta di lato, schivando abilmente la mossa di ${prev.playerVatsamon.name}!`);
      } else {
        ...
        logs.push(`⚔️ Il tuo ${prev.playerVatsamon.name} ${description} infliggendo ${finalDamage} danni!`);
      }
      ...
      if (nextOppHp <= 0) {
        ...
        let winLog = `🏆 RELLA D'ORO! ${prev.playerVatsamon.name} vince gloriosamente la Bataille de Reines! Ricevi +60 🪙 e +500 XP.`;
  ...
  const handleUseBackpackItemInCombat = (itemId: string) => {
    ...
    setGymState(prev => {
      if (!prev.playerVatsamon) return prev;
      const logs = [...prev.history];
      ...
      if (itemId === 'item-apple') {
        rawHeal = 150;
        msg = `🍎 Nutri ${prev.playerVatsamon.name} con una Mela Alpina d'Oro! Pura energia che ripristina +150 HP!`;
  ...
  const handleSwapCombatBuddy = (buddy: Vatsamon) => {
    if (gymState.status !== 'active' || gymState.activeTurn !== 'player') return;

    if (gymState.playerVatsamon?.id === buddy.id) {
    ...
    setGymState(prev => {
      const logs = [...prev.history];
      logs.push(`🔄 Richiami ${prev.playerVatsamon?.name} in panchina. Scende in campo l'inespugnabile ${buddy.name} (CP ${buddy.cp})!`);

      const newMaxHp = 320 + buddy.level * 18;

      const updatedState: BattleState = {
        ...prev,
        playerVatsamon: buddy,
  ...
  const processImageScanGo = async (imgBase64: string | null) => {
    ...
    try {
      const response = await fetch('/api/generate-vatsamon', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageBase64: imgBase64, isDemo: false })
      });

      const parsed: Vatsamon = await response.json();
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col font-sans text-slate-100 antialiased selection:bg-emerald-500 selection:text-white" id="vatsamon-go-app">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-mono font-black text-sm tracking-wide text-emerald-400">VATSAMON GO</span>
      ...
      <nav className="bg-slate-950/90 border-b border-slate-850 sticky top-[73px] z-40 shadow-inner">
        <div className="max-w-md mx-auto grid grid-cols-5 gap-0.5 p-1 text-[11px] font-extrabold">
          ...
          <button
            ...
            className={`flex flex-col items-center py-2 rounded-xl transition-all relative ${activeTab === 'vatsadex' ? 'bg-emerald-500 text-slate-950' : 'text-slate-400 hover:bg-slate-900'}`}
          >
            ...
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-3xl space-y-4 shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4" id="trekking-routes-selector">
                {TREK_ROUTES.map((route) => {
                  ...
                  const savedIdx = Number(localStorage.getItem(`vatsamon_waypoint_idx_${route.id}`) || 0);
                  const savedProg = Number(localStorage.getItem(`vatsamon_waypoint_progress_${route.id}`) || 0);
              ...
              <div className="flex-grow flex flex-col items-center justify-center relative my-4">
                ...
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  <div className={`transition-all duration-300 ${captureStep === 'wobbling' ? 'scale-0 translate-y-24 opacity-0 rotate-180 duration-[1200ms]' : captureStep === 'secured' ? 'opacity-0 scale-0' : 'animate-bounce'}`}>
                    <VatsamonAvatar breed={encounterCow.breed} rarity={encounterCow.rarity} className="w-36 h-36" />
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="my-2.5 flex justify-center">
                            <VatsamonAvatar breed={cow.breed} rarity={cow.rarity} className="w-20 h-20 group-hover:scale-110 transition-transform" />
                          </div>
            ...
            <div className="bg-slate-900 border-2 border-slate-800 rounded-3xl max-w-md w-full p-6 text-center space-y-4 shadow-2xl relative overflow-hidden">
              ...
              <div className="pt-2">
                <span className={`text-[10px] bg-slate-950 border font-mono font-black tracking-widest px-3 py-1 rounded-full uppercase ${
                  selectedVatsamon.rarity === 'Leggendaria' ? 'text-amber-400 border-amber-500/20' :
                  selectedVatsamon.rarity === 'Epica' ? 'text-purple-400 border-purple-500/20' :
                  selectedVatsamon.rarity === 'Rara' ? 'text-blue-400 border-blue-500/20' : 'text-slate-400 border-slate-800'
                }`}>
                  Bovina {selectedVatsamon.rarity}
                </span>
              </div>
              ...
              <div className="space-y-1">
                ...
                <h1 className="text-3xl font-mono font-black text-[#F5F5DC] tracking-tight leading-none uppercase">CP {selectedVatsamon.cp}</h1>
                <div className="text-xs text-slate-400 font-mono">Livello {selectedVatsamon.level} • Razza: {selectedVatsamon.breed}</div>
              </div>
              ...
              <div className="py-4 flex justify-center relative">
                <div className={`absolute inset-4 rounded-full pointer-events-none filter blur-xl ${
                  selectedVatsamon.rarity === 'Leggendaria' ? 'legendary-glow opacity-30' :
                  selectedVatsamon.rarity === 'Epica' ? 'epic-glow opacity-25' :
                  selectedVatsamon.rarity === 'Rara' ? 'rare-glow opacity-20' : 'opacity-0'
                }`}></div>
                <VatsamonAvatar breed={selectedVatsamon.breed} rarity={selectedVatsamon.rarity} className="w-32 h-32 relative z-10 animate-float" />
              </div>

              {/* RPG Stats curved gauges bars */}
              <div className="grid grid-cols-3 gap-2 py-2 bg-slate-950 rounded-2xl border border-slate-850">
                <div>
                  <span className="block font-mono font-black text-sm text-amber-400">{selectedVatsamon.stats.strength}</span>
                  <span className="text-[9.5px] text-slate-400 uppercase font-mono">Forza 🏋️‍♂️</span>
                </div>
                <div>
                  <span className="block font-mono font-black text-sm text-blue-400">{selectedVatsamon.stats.defense}</span>
                  <span className="text-[9.5px] text-slate-400 uppercase font-mono">Difesa 🛡️</span>
                </div>
                <div>
                  <span className="block font-mono font-black text-sm text-emerald-400">{selectedVatsamon.stats.agility}</span>
              ...
              <div className="bg-emerald-950/40 border-2 border-emerald-900 p-3 rounded-2xl text-left flex gap-1.5">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-slate-300 leading-normal">{selectedVatsamon.eco_tip}</p>
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {vatsadex.length > 0 ? (
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="bg-gradient-to-br from-slate-900 to-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3 relative overflow-hidden">
                              ...
                              <div className="flex items-center gap-3">
                                <VatsamonAvatar breed={activeBuddy.breed} rarity={activeBuddy.rarity} className="w-14 h-14" />
                                <div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar max-h-24">
                            {vatsadex.map(buddy => (
                              ...
                              >
                                <VatsamonAvatar breed={buddy.breed} rarity={buddy.rarity} className="w-8 h-8" />
                      ...
                      <div className="text-center p-6 bg-slate-900 rounded-2xl border border-slate-850 space-y-2">
                        <p className="text-xs text-slate-450">Non possiedi alcuna Reina nel Vatsadex!</p>
                    ...
                    <div className="flex flex-col items-end w-[90%] md:w-[75%] ml-auto max-w-sm">
                      <div className="bg-slate-900 p-3 rounded-2xl border border-slate-850 w-full relative overflow-hidden flex flex-col gap-1.5 shadow-md">
                        ...
                        <div className="flex justify-between items-center text-xs font-mono font-bold pb-1 text-slate-200">
                          <span className="text-red-400 leading-none truncate max-w-[120px]">👑 BOSS {gymState.opponentVatsamon?.name}</span>
                          <span className="text-[10px] bg-red-950 border border-red-900 text-red-300 px-1 py-0.2 rounded font-black text-right scale-95 uppercase">
                            LV {gymState.opponentVatsamon?.level}
                          </span>
                        </div>
                      ...
                      <div className="mr-6 mt-1.5 flex justify-end">
                        <div className={`transition-all duration-150 ${gymState.opponentAttackAnim ? '-translate-x-12 scale-110 -rotate-6 z-10 filter brightness-110' : ''}`}>
                          <VatsamonAvatar breed={gymState.opponentVatsamon!.breed} rarity={gymState.opponentVatsamon!.rarity} className="w-16 h-16 filter drop-shadow-[0_4px_12px_rgba(239,68,68,0.25)]" />
                    ...
                    <div className="flex flex-col items-start w-[90%] md:w-[75%] mr-auto max-w-sm">
                      ...
                      <div className="ml-6 mb-1.5 flex h-[70px] items-end justify-start">
                        <div className={`transition-all duration-150 ${gymState.playerAttackAnim ? 'translate-x-12 scale-110 rotate-6 z-10 filter brightness-110' : ''}`}>
                          <VatsamonAvatar breed={gymState.playerVatsamon!.breed} rarity={gymState.playerVatsamon!.rarity} className="w-16 h-16 filter drop-shadow-[0_4px_12px_rgba(16,185,129,0.25)]" />
                          <div className="w-16 h-1 rounded-full bg-slate-900/60 blur-[2px] mt-0.5 mx-auto"></div>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-900 p-3 rounded-2xl border border-slate-850 w-full relative overflow-hidden flex flex-col gap-1.5 shadow-md">
                        ...
                        <div className="flex justify-between items-center text-xs font-mono font-bold pb-1 text-slate-200">
                          ...
                          <span className="text-[10px] bg-emerald-950 border border-emerald-900 text-emerald-300 px-1 py-0.2 rounded font-black text-right scale-95 uppercase">
                            LV {gymState.playerVatsamon?.level}
                          </span>
                        </div>
                          ...
                          {battleActionTab === 'swap' && (
                            <div className="space-y-2.5 max-h-[160px] overflow-y-auto no-scrollbar" id="partner-switchboard">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                {vatsadex.map(buddy => {
                                  const isCurrentFighter = gymState.playerVatsamon?.id === buddy.id;
                                  return (
                                    <button
                                      key={buddy.id}
