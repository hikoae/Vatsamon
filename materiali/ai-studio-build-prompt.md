...
Incolla tutto questo nel campo di Build. Poi carica `data/vazzadex.json` come dato dell'app.

---

Crea una **Progressive Web App mobile-first in italiano** chiamata **Vazzamon**, in stile
Pokémon GO ma con le mucche reali delle Bataille de Reines della Valle d'Aosta.

DATI: usa il file `vazzadex.json` che carico. Contiene `bovine` (53 schede con: id, nome,
...
2. **Incontro**: mostra la silhouette (usa assets/cow-silhouette.svg se manca la foto),
   ...
   - Se razza riconosciuta → rivela la SCHEDA della bovina e salvala nella Vazzadex.
...
3. **Vazzadex**: griglia delle bovine catturate (salvate in localStorage). Ogni cella:
   silhouette/foto, nome, stelle di rarità (★), tipo. In alto: "Vazzadex: X / 53".
